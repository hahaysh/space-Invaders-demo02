...
| R11 P 전환 (07-02 추가 예정) | playing/paused 왕복, title/won/lost 무시, 상태 외 전체 데이터 보존 | 실제 P 정지/재개·DOM 안내, P repeat 무시, title/won/lost P 무시, paused Enter/R 무시 |
| R12 동결/재개 (07-02 추가 예정) | paused 전체 중첩 상태 불변, elapsed/cooldown 보존, 남은 발사 간격 경계 | 정지 전후 Canvas·점수 불변, 전환 입력 제거, 첫 재개 프레임·반복 전환 후 속도·발사 회귀 |
...
## 07-02 일시정지 검증 설계 (미실행)

07-01은 문서 검토만 수행한다. 다음 별도 실행 지시에서 기존 R01~R10 회귀를 유지하고 R08/R09에 paused를 추가하며 R11/R12를 검사한다.
