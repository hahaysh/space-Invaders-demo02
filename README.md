# 우주 방어

좌우로 이동하며 적 편대를 제거하고 방어선을 지키는 데스크톱 브라우저 게임입니다.
HTML/CSS·ES Modules·Canvas 2D로 구현했으며 외부 이미지나 CDN 없이 실행합니다.

**공개 URL: <https://hahaysh.github.io/space-Invaders-demo02/>**
2026-09-14 첫 공개의 응답·자산·자동 브라우저 조작을 확인했습니다. 사람의 직접 플레이와 UI 승인은 미확인이며, [실제 공개 근거](https://github.com/hahaysh/space-Invaders-demo02/issues/4#issuecomment-5659428787)와 [결과 요약](TEST_RESULTS.md)을 구분해 기록합니다.
기록 PR #6 병합 후 동일 게임의 [직전 공개 최종 확인·06-03 완료 근거](https://github.com/hahaysh/space-Invaders-demo02/issues/4#issuecomment-5659560168)를 이어받습니다.
이후 일시정지 PR #8의 main 배포와 실제 공개 P 조작은 [07-02 완료·직전 공개 근거](https://github.com/hahaysh/space-Invaders-demo02/issues/7#issuecomment-5659849823)로 확인되었습니다. 이번 문서 단계에서 공개 검사를 다시 실행한 것은 아닙니다.
난이도 PR #10의 main 배포·공개 자산 일치·실제 선택/잠금/P 조작은 [08-02 완료·직전 공개 근거](https://github.com/hahaysh/space-Invaders-demo02/issues/9#issuecomment-5660296207)에 보존되어 있습니다. 아래 조작은 현재 코드 기준입니다. 목숨·재도전의 병합 후 최종 공개 결과는 [이슈 #11 단계 댓글](https://github.com/hahaysh/space-Invaders-demo02/issues/11)에서 확인하며, 직전 난이도 공개 근거와 구분합니다.

## 준비와 설치

검증 환경은 Windows, Node.js `v24.14.1` LTS, npm `10.8.3`, Playwright Chromium입니다.
다른 OS/브라우저와 사람의 직접 플레이는 아직 확인하지 않았습니다.

저장소를 내려받은 후 `package.json`이 있는 폴더에서 실행합니다.

```powershell
npm ci --no-fund --no-audit --registry=https://registry.npmjs.org
```

고정된 의존성은 `package-lock.json`으로 설치합니다. 일반 게임 실행에는 Playwright 브라우저 설치가 필요하지 않습니다.
브라우저 검사를 처음 실행하는 환경에서 Chromium이 없다는 오류가 나오면 다음 명령으로 설치합니다.

```powershell
npm run browser:install
```

현재 검증 환경에는 Chromium이 있어 위 설치 명령을 별도로 실행하지 않았습니다.

## 로컬 실행

```powershell
npm run dev
```

브라우저에서 <http://127.0.0.1:5173/>를 엽니다. 서버는 명령을 실행한 터미널에서 유지되며 `Ctrl+C`로 종료합니다.
이미 같은 게임 서버가 실행 중이면 재사용하고 중복 실행하지 마세요.
포트가 사용 중이면 다른 포트로 자동 변경하지 않고 실패합니다. 소유한 서버인지 확인한 뒤 그 서버만 종료하고, 모르는 프로세스는 종료하지 마세요.

## 조작과 목표

| 동작 | 조작 |
|---|---|
| 다음 판 난이도 선택 | 시작·승리·패배 화면의 기본 선택 상자 |
| 게임 시작 | 시작 버튼 또는 Enter (난이도 선택 중 제외) |
| 좌우 이동 | ← / → 또는 A / D |
| 연속 발사 | Space 길게 누르기 |
| 진행 중 일시정지·재개 | P (길게 눌러도 한 번만 전환) |
| 재도전 대기에서 다음 시도 | 다음 시도 버튼 또는 Enter (반복 입력 제외) |
| 승리·패배 후 새 게임 | 다시 시작 버튼 또는 R |

반대 방향을 함께 누르면 정지합니다. 적을 맞히면 제거되고 점수가 올라갑니다.
모든 적을 제거하면 남은 목숨을 유지한 채 승리합니다. 생존 적이 점선 방어선에 닿으면 목숨을 잃습니다.
목숨이 남으면 **재도전 대기**, 모두 소진하면 **패배**입니다. 같은 갱신에 여러 적이 도달해도 한 번만 차감합니다.
실패 직후에는 그 시도의 화면·점수·시간을 유지하며 자동으로 다음 시도를 시작하지 않습니다.
**다음 시도**는 남은 목숨과 현재 난이도만 유지하고 점수·편대·플레이어·탄환·방향·발사 간격·시간·입력을 초기화합니다. 점수는 시도 사이에 누적하지 않습니다.
종료 후 **새 게임**은 목숨까지 초기화하고 선택 난이도를 새로 적용합니다. 초기 목숨과 정확한 규칙은 [PRD](PRD.md)를 따릅니다.
진행 중 또는 일시정지 중 Enter/R은 게임을 초기화하지 않습니다.
재도전 대기에서는 R/P·이동·발사·반복 Enter를 무시합니다. 다음 시도를 시작한 뒤 이동·발사를 하려면 누른 키를 떼고 새로 누르세요.

**다음 판 난이도**에서 쉬움·보통·어려움을 고르면 시작/재시작 때만 현재 판에 적용됩니다.
옆의 **현재 판**은 실제 적용된 난이도이며, 종료 화면에서 다음 난이도를 골라도 이전 판의 표시는 바뀌지 않습니다.
진행 중·일시정지·재도전 대기에서는 선택을 잠그고, P로 재개하거나 다음 시도를 시작해도 같은 난이도를 유지합니다.
같은 페이지의 다음 판은 선택을 유지하지만 새로고침하면 보통으로 돌아갑니다. 설정을 저장하지 않습니다.
난이도는 적의 수평 속도만 바꾸며 적 수·점수·하강량·승패 규칙은 같습니다.

Tab으로 선택 상자에 접근해 화살표로 항목을 고르고 Space/Enter로 기본 선택 메뉴를 조작할 수 있습니다.
선택 상자에 포커스가 있을 때 Enter는 게임 시작이 아닙니다. 선택을 확정한 뒤 Tab으로 시작/재시작 버튼에 이동해 Enter 또는 Space로 실행하세요.
잘못된 난이도 값은 오류 안내로 표시하며 기존 선택과 판을 유지합니다. 보통으로 조용히 대체하지 않습니다.

P는 진행 중/일시정지에서만 동작하며 시작·재도전 대기·승리·패배 화면에서는 무시합니다.
일시정지 중에는 화면의 게임 장면·점수·게임 시간·남은 발사 간격이 유지됩니다.
정지/재개 때 누른 키는 비우므로 이동·발사를 계속하려면 해당 키를 떼었다가 다시 누르세요.
재개는 새 게임이 아니라 같은 상태에서 이어 하기이며 정지한 시간만큼 이동·발사가 몰리지 않습니다.

포커스를 잃거나 페이지가 비표시되면 누른 키를 비웁니다. 자동 일시정지는 하지 않습니다.
위 P 조작은 [이슈 #7](https://github.com/hahaysh/space-Invaders-demo02/issues/7)에서 구현·공개되었습니다.
[이슈 #9](https://github.com/hahaysh/space-Invaders-demo02/issues/9)의 난이도 선택은 구현·공개되었으며 실제 공개 결과는 위 08-02 근거를 따릅니다.
[이슈 #11](https://github.com/hahaysh/space-Invaders-demo02/issues/11)의 목숨·재도전은 승인된 [PRD R17~R20](PRD.md)에 따라 현재 코드에 구현했습니다. 코드 수용·로컬/CI 검증과 최종 공개 완료는 구분하며 마지막 공개 근거는 해당 이슈 댓글에 보존합니다.
적 공격·모바일 터치 조작·사운드는 계속 범위 밖입니다.
정확한 수치와 판정 우선순위는 [PRD](PRD.md)를 참조하세요.

## 검사

모델 규칙 검사는 서버 없이 실행할 수 있습니다.

```powershell
npm test
```

실제 Chromium 입력·DOM·Canvas 검사는 **이 저장소의 dev 서버가 위 주소에서 실행 중인 상태**로 별도 터미널에서 실행합니다.
이 검사는 이미 실행 중인 서버를 대상으로 하므로 다른 프로젝트의 서버가 아닌지 먼저 확인하세요.

```powershell
npm run test:e2e
```

Node 검사는 경계·충돌·승패 우선순위·시간·초기화와 일시정지 상태 보존·남은 발사 간격을 확인합니다.
Chromium 검사는 버튼·키보드 입력으로 시작·이동·발사·자연 승패·종료 정지·재시작과 P 정지/재개·입력 정리·정상 속도를 확인합니다.
난이도는 모델의 정밀 시간·속도·pending/current·상태 잠금·invalid 오류와 Chromium의 실제 선택 키·DOM/Canvas·새로고침을 함께 검사합니다.
목숨은 동시 도달·반복 차감 방지·충돌 우선·전체 초기화 모델 경계와 실제 재도전 Enter/버튼·입력/난이도 잠금·Canvas 동결·최종 패배/새 게임을 함께 검사합니다.
시간 제어에는 Playwright clock을 사용하고, 일부 입력 정리/반복키 검사는 합성 이벤트를 사용합니다.
자동 검증을 사람의 직접 플레이나 실제 OS 탭 전환 확인으로 간주하지 않습니다.
실제 실행 결과와 미확인은 [구현 계획의 단계별 기록](IMPLEMENTATION_PLAN.md)에 있습니다.

## 빌드와 로컬 배포본 확인

```powershell
npm run build
npm run preview
```

빌드 결과는 `dist`에 생성됩니다. preview 주소는 <http://127.0.0.1:4173/>이며 사용 후 해당 터미널에서 `Ctrl+C`로 종료합니다.
preview는 로컬 빌드 확인용이며 공개 배포가 아닙니다. `dist`, `node_modules`, 테스트 생성물은 Git에 넣지 않습니다.

## GitHub Copilot App 수동 명령

[`.github/github-app.yml`](.github/github-app.yml)은 다음 두 수동 명령만 정의합니다.

| 이름 | 명령 | 범위 |
|---|---|---|
| Run | `npm run dev` | 로컬 개발 서버 |
| Test | `npm test` | Node 모델 검사만 실행 |

자동 설치·삭제·세션 트리거는 없습니다. 의존성 설치와 Chromium 검사는 위 명령을 별도로 실행합니다.
Run을 실행하기 전에 같은 서버가 이미 있는지 확인하세요.

App이 현재 파일의 설정 검토 화면을 제공하면 명령을 읽고 review/accept한 후 사용해야 합니다.
파일이 외부에서 바뀌면 공백·주석 변경만 있어도 다시 검토해야 하며, 그 전에는 이전 설정을 사용할 수 있습니다.
명령은 App의 GitHub 자격 증명을 상속할 수 있으므로 환경 변수·토큰을 출력하거나 저장하도록 변경하지 마세요.

**현재 설정 trust review/accept와 App Run UI 동작은 미확인입니다.**
이 환경에서 App UI 조회가 도구에 의해 거부되어 우회하지 않았습니다. 터미널에서 같은 명령이 성공한 사실과 App 적용 여부는 구분합니다.
설정 형식과 신뢰 절차는 [공식 문서](https://docs.github.com/copilot/reference/github-copilot-app-reference/repository-configuration)를 따릅니다.

## 문서

- [ideation.md](ideation.md): 대상과 재미
- [PRD.md](PRD.md): 제품 수치·규칙·수용 기준
- [TRD.md](TRD.md): 기술 선택과 검증 책임
- [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md): 단계별 진행·실제 결과·남은 작업
